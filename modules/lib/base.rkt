...
;; generated by a macro. All the modules are defined with
;; module+. This just means that the submodule can reference the
;; parent module and that it can be written in pieces (all the module+
;; forms are concatenated together to get one submodule). Both of
;; these points are essential. For instance, the actual type is
;; defined in the parent module and the 'types' submodule contains a
;; list of those actual types, so it has to reference it. As for being
;; written in pieces, we don't know beforehand what the types are
;; going to be, so we add them piecemeal in little additions to the
;; 'types' submodule.
;;
;; Go to (B)
